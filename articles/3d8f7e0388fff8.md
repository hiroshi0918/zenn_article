...
3. `documents/{doc_id}?type=5` で CSV ZIP を取得する
...
`type=2` や `type=5` は、API 全体で共通の意味を持つ番号ではありません。どの endpoint に対して呼ぶかで意味が変わります。
...
:::message
リポジトリの正規の入口は `edinet` CLI ですが、記事の流れを追いやすくするために互換用の入口ファイルも載せておきます。

- [`src/fetch_edinet.py`](https://github.com/hiroshi0918/edinet_data_pipeline/blob/main/src/fetch_edinet.py)
- [`src/extract_metrics.py`](https://github.com/hiroshi0918/edinet_data_pipeline/blob/main/src/extract_metrics.py)
:::
